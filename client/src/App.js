...
import { hideLoader, showLoader } from './components/actions/loader';
...
function App() {
  ...
  const getWeather = async (e) => {
    ...
    if (city) {
      ...
      const forecastRes = await Api.getForecast(data.coord.lat, data.coord.lon);
      setTimeout(() => {
        dispatch(hideLoader());
      }, 900);
      setState({
